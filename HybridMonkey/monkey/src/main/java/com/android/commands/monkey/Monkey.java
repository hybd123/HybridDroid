...
import com.bytedance.fastbot.opencvClient;
...
public class Monkey {
    static {
        try {       // 加载 OpenCV 核心库
            opencvClient.loadLibraryFromJar("opencv_java4");
        } catch (Exception e) {
            Logger.println("OpenCV load failed: " + e);
            System.exit(1);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Logger.println("load opencv success!");
...
//            ((MonkeySourceRandom) mEventSource).setAttr(mMainApps.get(0).getPackageName(), appVersionCode);
            ...
            ((MonkeySourceRandom) mEventSource).setAttribute(mMainApps.get(0).getPackageName(), appVersionCode, mMainIntentAction, mMainIntentData, mMainQuickAppActivity);
            for (int i = 0; i < MonkeySourceRandom.FACTORZ_COUNT; i++) {
                if (mFactors[i] <= 0.0f) {
        ...
        while (!systemCrashed) {
            ...
            if (ev != null) {
                ...
                if (!(ev instanceof MonkeyThrottleEvent)) {
                    eventCounter++;
                    // xmq added to diff LLM event
//                    Logger.println("EventSource: "+ev.getEventSource());
                    if (mCountEvents) {
                        cycleCounter++;
                    }
                }
    ...
    private static String getOpenCVLocally(){
        String[] abis = Build.SUPPORTED_ABIS;
        List<String> abisList = Arrays.asList(abis);
        if (abisList.contains("x86_64")) {
            return "/data/local/tmp/x86_64/libopencv_java4.so";
        } else if (abisList.contains("x86")) {
            return "/data/local/tmp/x86/libopencv_java4.so";
        } else if (abisList.contains("arm64-v8a")) {
            return "/data/local/tmp/arm64-v8a/libopencv_java4.so";
        } else {
            return "/data/local/tmp/armeabi-v7a/libopencv_java4.so";
        }
    }

    private static String getOpenCVPathFromAPK(){
        String[] abis = Build.SUPPORTED_ABIS;
        List<String> abisList = Arrays.asList(abis);
        if (abisList.contains("x86_64")) {
            return "/data/local/tmp/monkey.apk!/lib/x86_64/libopencv_java4.so";
        } else if (abisList.contains("x86")) {
            return "/data/local/tmp/monkey.apk!/lib/x86/libopencv_java4.so";
        } else if (abisList.contains("arm64-v8a")) {
            return "/data/local/tmp/monkey.apk!/lib/arm64-v8a/libopencv_java4.so";
        } else {
            return "/data/local/tmp/monkey.apk!/lib/armeabi-v7a/libopencv_java4.so";
        }
    }

    private static String getOpenCVPath(boolean fromAPK) {
        if(Build.VERSION.SDK_INT<=Build.VERSION_CODES.LOLLIPOP_MR1) {
            return getOpenCVLocally();
        }else {
            if (fromAPK) {//todo
                return getOpenCVPathFromAPK();
            }else {
                return getOpenCVLocally();
            }
        }
    ...
    private static boolean tryToLoadNativeLib(boolean fromAPK){
        String path = "";
        try {
            path = getOpenCVPath(fromAPK);
            System.load(path);
            Logger.println("opencv native : library load!");
            Logger.println("opencv native path is : "+path);
